...
tryCatch(scidbrm(scidbls()),error=invisible,force=TRUE)
@
...
database. See the help page for \verb+redimension+ and this example:

<<>>=
# Redimension aggregation example (cf. above example)
redimension(df,
            dim="Species",
            FUN="avg(Petal_Length) as Petal_Length_avg,
                     stdev(Petal_Width) as Petal_Width_stdev")[]
@
