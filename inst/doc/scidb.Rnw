...
Many interesting SciDB features include ACID transactions, parallel processing,
...
coordinator instance. The function may be safely called multiple times and.
...
SciDB share a number of common data types, each system contains types not
...
library("scidb")
scidbonnect()               # Connect to SciDB on localhost
...
outlined in detail in the R help page, \verb+?df2scidb+.
...
sparse elements are implicitly zero).
...
attributes}, or variables. The number and data types of attributes are 
...
The integer co\"ordinate systems used to index SciDB arrays are similar to R,
except that SciDB integer indices may be zero or negative, and are represented
by 62-bit signed integers.
...
Double-precision floating point values in SciDB also provide a value indicating
missing values identically to R, and use the identical {\tt NA} representation
that R uses. Other SciDB data types do not define {\tt NA}. Use of {\tt NA}
with other than double-precision floating point values is undefined.


\section{The {\tt scidbdf} data.frame-like class}

The \verb+scidbdf+ class defines a data frame-like class with data backed
by 1D SciDB arrays. Like data frames, the columns may be of distinct types
and the rows represent observations. Each attribute in  the backing SciDB
array represents a column in the \verb+scidbdf+ object. Objects are
read-only, although new \verb+scidbdf+ objects can be created.
...
XXX
 ...
 $ bool     : chr  "true" "false"
 $ char     : chr  "x" "x"
 $ datetime : chr  "1970-01-01 00:00:01" "1970-01-01 00:00:02"
 $ double   : num  1 1.41
 $ float    : num  1 1.41
 $ int16    : int  1 2
 $ int32    : int  1 2
 $ int64    : num  1e+11 1e+11
 $ string   : int  1 2
 $ string_10: int  1 2
 $ uint16   : int  1 2
 $ uint32   : int  1 2
