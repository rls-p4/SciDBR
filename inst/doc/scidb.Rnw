...
optionally showing detailed schema information for arrays. The
\verb+scidblist+ function accepts regular expression-style filtering
options to limit the returned results.
...
\end{lstlisting}
\end{Code}
After a while, the algorithm
returns the truncated SVD in the variable S. The result obtained is
...
\subsection{Package options and miscellaneous notes}
