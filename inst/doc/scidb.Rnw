...
attributes}, or variables. The number and data types of attributes are uniform
...
In addition toavailable missing (NULL) codes, SciDB double-precision floating
...
\verb+tcrossprod+) are directly with R syntax for \verb+scidb+ array objects.
...
The \verb+scidb+ class supports the following operations:
...
The subarray operator \verb+[ ]+ supports the standard \verb+drop+ argument and
...
The truncagted singular value distribution (TSVD) is an important, widely used
...
entries (consuming about $18\,$GB).  That problem is large enough that it can't
be computed in R--the matrix is too large to represent in R.  (Note that if a
SciDB array named {\tt A} already exists, either delete it before running the
following example, or change the name used in the example.)
...
\end{lstlisting}
\end{Code}
After a while, depending on the system SciDB is running on, SciDB
...
it could handle the large matrix. Further optimizations are possible, but
this simple example shows that it can be easy to get large-scale computation
working without rewriting R code.
...
network service on the SciDB database coo\"rdinator computer.
...
service called \verb+shim+ on the computer that SciDB is installed on. This
service only needs to be installed on the SciDB machine, not on client
computers that connect to SciDB from R.  It's available in packaged binary form
for supported SciDB operating systems, and as source code which can be compiled
and deployed on any SciDB installation.

Both installation approaches install the \verb+shim+ network service on the
SciDB co\"ordinator computer. Installing as a service requires root permission.
...
Paradigm4's laboratory on github at 
\newline
...
for source code. The open source package author, Bryan Lewis, presently maintains
...
{https://github.com/Paradigm4/shim/} for more information and source code.
...
co\"ordinator machine and port 8080, for example:
\href{http://localhost:8080}{http://localhost:8080} on the co\"ordinator
