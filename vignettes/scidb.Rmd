...
more values called _attribtues_ of distinct types at each coordinate.
...
we only actively test agains the current release version of the database.
...
function acceps arbitrary SciDB AFL expressions or named SciDB arrays.
...
Or, we can equivalently use a special empty-indexing operator `x[]` on any
...
values in R. With few excpetions, operations on SciDB array objects produce new
...
Use `project`, or equivalently the data frame-like column subsetting notations
`$` and `x[, c("column name 1", "column name 2"]` to  project the array object
onto a subset of its SciDB attributes.
...
The implict form can sometimes generate more efficient queries and
...
```
```
SciDB expression  cross_join(project(R_array1ebfe561a...
SciDB schema  <Petal_Length:double NULL DEFAULT null,Petal_Width:double NULL DEFAULT null> [row=1:150,150,0,Species_index=0:*,6667,0] 
       variable dimension   type nullable start end chunk
1           row      TRUE  int64    FALSE     1 150   150
2 Species_index      TRUE  int64    FALSE     0   *  6667
3  Petal_Length     FALSE double     TRUE                
4   Petal_Width     FALSE double     TRUE                
```

# Other SciDB operators

# Package options

# Extras

# Special note on the `curl` pacakge dependency
