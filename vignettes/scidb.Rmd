...
# Uploading R values to SciDB with `as.scidb()` and downloading SciDB
  values to R with `as.R()`
...
Factor values are uploaded as their corresponding level character
strings.

The `as.scidb()` function uploads returns a reference to a SciDB array
containing the uploaded data. The following example uploads a data frame
to SciDB, warning us that variable names were changed, and then downloads
the resulting SciDB object data back into R.
