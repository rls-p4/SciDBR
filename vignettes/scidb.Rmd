...
to any potential namespace-setting prefix expression). And the object itself
is really a list that contains available SciDB AFL operator and macro functions.

```{r, eval=FALSE}
db <- scidbconnect()
print(db)    # summarize connection
ls(db)       # quick list of arrays
# db$<TAB>     # list of AFL operators
```
...
AFL expressions from R, discussed in more detail in the next section.
...
2. Composing AFL expressions from the database connection AFL function list

Use the `iquery` function to run arbitrary SciDB AFL queries entered as
character strings, optionally returning results to R as a data frame.
