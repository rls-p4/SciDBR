...
The `as.scidb()` function uploads returns a reference to a SciDB array
...
Note that in the above expression `i` and `j` are evaluated as SciDB dimension
values, not R values.  The expression evaluation procedure is:
...
SciDB schema literal values represent one important exception to the above
process.  SciDB schemas contain characters like `<`, `:`, `[` that correspond
to R infix operators. It's difficult to always suppress their evaluation as
operators. Thus SciDB schemas must be supplied as R character strings as shown
in the following example (also used above).
```{r, eval=FALSE, echo=TRUE}
x <- db$build("<v:double>[i=1:2,2,0, j=1:3,1,0]", i * j)
```
...
```{r, eval=FALSE, echo=TRUE}
db <- scidbconnect()
x <- as.scidb(iris)     # upload the iris data frame to SciDB
...
all the issued SciDB queries. Set `options(scidb.stream=TRUE)` to avoid
creating server-side data files which is most useful when you're downloading
lots of data from SciDB into R.
