...
output:
  ...
  html_document:
    ...
    css: style.css
vignette: |
  %\VignetteIndexEntry{scidb} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
---
...
```{r, eval=FALSE}
db <- scidbconnect(port=8083, username="root", password="Paradigm4", protocol = 'https')
```
*Connect to shim on an encrypted port 8083 with example SciDB authentication, interactively entering password*
```{r, eval=FALSE}
db <- scidbconnect(port=8083, username="root", password=getpwd(), protocol = 'https')
```
