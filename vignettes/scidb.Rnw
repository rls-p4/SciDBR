...
\raggedleft
\includegraphics{p4logo}$\qquad\qquad$
\includegraphics[width=0.4\textwidth]{scidblogo}$\qquad\qquad$
\includegraphics[width=0.2\textwidth]{Rlogo}
\end{minipage}
...
when requested.
\item By running SciDB queries from R, optionally returning results as data
frames.
\end{enumerate}
...
The network interface optionally supports SSL encryption and PAM or HTTP digest
user authentication.  The shim service can be configured to support either
...
{\bf Connect to SciDB on an encrypted port 8083 with example PAM authentication:}
\begin{Schunk}
\begin{Sinput}
...
The \verb+scidbremove+ function removes a SciDB array, or optionally a set of
SciDB arrays. The function accepts a vector of array names, resulting in the
removal of all the specified arrays. Combine this feature with the regular
expression filtering output of \verb+scidblist+ to remove sets of arrays
matching the filter.  Array names not associated with a specific R session are
protected from easy removal to provide a modicum of protection for multiple
SciDB users.  Specify \verb+force=TRUE+ as indicated in the warning message to
remove those arrays.
...
attribute separately. A one-dimensional SciDB array is conceptually
...
Higher-dimensional arrays in SciDB are generalizations of data frames and they
don't correspond directly to objects in R; the {\tt scidb} $n$-dimensional
array class described below is sometimes limited to working with one attribute
at a time to more closely emulate standard R arrays.
...
SciDB schema:  <Expt:int32 NULL DEFAULT null,Run:int32 NULL DEFAULT null,Speed:int32 NULL DEFAULT null> [row=1:100,100,0]
...
\end{Sinput}
\begin{Soutput}
SciDB expression:  R_array366d7949bc241102999947615
...
\begin{Soutput}
            0           1          2           3          4
0  2.80783239 -0.05946079  0.6109525  1.53758978 -1.1351279
1 -0.05946079  0.37437931  0.4675420 -0.08090686  0.8351372
2  0.61095249  0.46754196  2.9929278 -2.32618189  0.7360878
3  1.53758978 -0.08090686 -2.3261819  5.48612039 -0.5832051
4 -1.13512788  0.83513722  0.7360878 -0.58320510  2.5291922
\end{Soutput}
\begin{Sinput}
...
SciDB arrays are indexed by signed 62-bit integers. The origin of an array
depends on its schema and can be any valid integer coordinate. However, zero is
a typical origin, and some SciDB operations require zero-based indexes.

By contrast, R indices use positive 52-bit integer-valued double precision
floating point numbers.  That means that SciDB arrays can't always be indexed
by R. In practice, R users should avoid creating SciDB arrays with coordinate
systems that extend past R's limits. Moreover, all R array coordinate systems
...
Be aware that SciDB array coordinate systems may not begin at 1. Array
coordinates are displayed when data are downloaded, and the underlying
coordinate system can be inspected using the \verb+str+ or \verb+schema+
functions.
...
\verb+schema+, or \verb+dimensions+ functions to interrogate the SciDB object
for details.
...
\item The bracket subsetting function \verb+[+ takes an optional logical
\verb+redim+ argument. When \verb+FALSE+, the returned value has the same
shape as the input value, with regions outside the indicated range marked
sparse (empty). See the examples below.
\item Subsetting operations using the bracket \verb+[+ function return
zero-indexed results unless the \verb+redim=FALSE+ argument is specified.
...
or string indexing.
\item 
All array types support labeled dimension indexing using the R \verb+rownames+,
\verb+colnames+, or \verb+dimnames+ and related functions. Labels assigned in this
way must be provided by 1-d SciDB arrays that map the integer coordinates to
character label values. See the examples below.
\item Arrays may be indexed by other SciDB arrays
...
instead of \verb+[1:1e9]+. Use \verb+between(0,Inf)+, for example, to indicate
every index greater than or equal to zero.
...
Let's illustrate some indexing ideas by example. Consider the 5x4 matrix
with unusual origin of $(-2,0)$:

\begin{Schunk}
\begin{Sinput}
> (x <- as.scidb(matrix(as.double(1:20),nrow=5), start=c(-2,0)) )
\end{Sinput}
\begin{Soutput}
...
The underlying coordinates are displayed when data are returned to R
using the special empty index function:
\begin{Schunk}
\begin{Sinput}
> x[]
\end{Sinput}
...
The package adopts the convention that explicit subsets return zero-indexed
results, noting also that zero and negative integers are valid SciDB indexes
(deviating from R's usual interpretation):
\begin{Schunk}
\begin{Sinput}
> x[0:2, 2:3] []
\end{Sinput}
...
Use the optional \verb+redim=FALSE+ option to return a masked sparse
array of the same shape as the input. This is often useful in conjunction
with database join operations.
\begin{Schunk}
\begin{Sinput}
> x[0:2, 2:3, redim=FALSE] []
\end{Sinput}
\begin{Soutput}
...
The bracket function also supports the standard R \verb+drop+ argument:
\begin{Schunk}
\begin{Sinput}
> x[, 1] []
\end{Sinput}
...
Non-contiguous selections are supported, noting again that returned subset
results by convention here are zero-indexed:
\begin{Schunk}
\begin{Sinput}
> x[c(0,2,-1), ][]
\end{Sinput}
...
\end{Soutput}
\end{Schunk}
(However, unlike standard R array subsetting repeated indices are not
supported.)

Coordinate axes may be labeled:
\begin{Schunk}
\begin{Sinput}
...
\end{Soutput}
\end{Schunk}
The labels are maintained as ancillary SciDB arrays under the hood. You can
view them:
\begin{Schunk}
\begin{Sinput}
> dimnames(x)
...
\begin{Soutput}
[[1]]
Reference to a SciDB vector of length 5 

[[2]]
Reference to a SciDB vector of length Inf 
\end{Soutput}
\begin{Sinput}
> dimnames(x)[[1]] []
\end{Sinput}
...
\end{Soutput}
\end{Schunk}
If a sparse array has coordinates too large for R to manage, results are
returned in an ``unpacked'' data frame form.

Note that SciDB objects with coordinates beyond the range of R display as
\verb+inf+. Coordinate labeling arrays are sparse with a very large upper
coordinate bound by convention--allowing the number of labels to grow easily.

Labeled arrays can be subset, and the result retains labels:
\begin{Schunk}
\begin{Sinput}
> x[c('c','a','e'), c('B','A')] []
\end{Sinput}
...
Data-frame like objects are supported. Under the hood they are 1-d SciDB
arrays.
\begin{Schunk}
\begin{Sinput}
> d <- as.scidb(iris)
...
\end{Soutput}
\end{Schunk}
Note that the package will attempt to rename variables to conform to SciDB
naming conventions when it has to.

The ``columns'' of a data frame like object are defined by the SciDB array
attributes. You can use numeric or named indices, or the R dollar sign
notation. The following subsets are all the same:
\begin{Schunk}
\begin{Sinput}
> d$Petal_Width
\end{Sinput}
\begin{Soutput}
Reference to a SciDB vector of length 150 
\end{Soutput}
\begin{Sinput}
> d[, "Petal_Width"]
\end{Sinput}
...
3 x 3 sparse Matrix of class "dgCMatrix"
          0         1         2
0 .         1.5952808 0.4874291
1 0.1836433 0.3295078 0.7383247
2 .         .         0.5757814
\end{Soutput}
\begin{Sinput}
...
\end{Sinput}
\begin{Soutput}
  row_2 nationality deceased     name                         title
1     0   Australia       no   McNeil     Interactive Data Analysis
2     1          UK       no   Ripley            Spatial Statistics
3     2          UK       no   Ripley         Stochastic Simulation
4     3          US       no  Tierney                     LISP-STAT
5     4          US      yes    Tukey     Exploratory Data Analysis
6     5   Australia       no Venables Modern Applied Statistics ...
  other_author
1         <NA>
2         <NA>
3         <NA>
...
that aggregation functions must generally be valid SciDB aggregate expressions,
...
Alternatively, use the SciDB-specific \verb+redimension+ function to perform
aggregations. This form of aggregation is sometimes more efficient in the
database. See the help page for \verb+redimension+ and the examples in the
following section on \verb+redimension+.
...
\end{Sinput}
\begin{Soutput}
  Sepal_Width Petal_Length Petal_Width Species prod
1         3.5          1.4         0.2  setosa 0.28
2         3.0          1.4         0.2  setosa 0.28
3         3.2          1.3         0.2  setosa 0.26
\end{Soutput}
\end{Schunk}


\subsection{SciDB redimension}

The \verb+redimension+ function is a wrapper for the SciDB \verb+redimension+
operator--the Swiss army knife of SciDB. The \verb+redimension+ function
can be used to change the dimensionality of an array and even to apply
reduction functions while doing so to achieve aggregations.

The following example illustrates using \verb+redimension+ to compute
a congingency table along levels of two variables, "class" and "Species."
\begin{Schunk}
\begin{Sinput}
> x <- as.scidb(iris)
...
> redimension(y, dim=c("class", "Species"), FUN=count) []
\end{Sinput}
\begin{Soutput}
...
\end{Soutput}
\begin{Sinput}
> # Aggregation example (cf. aggregation example above)
> redimension(x,
+             dim="Species",
+             FUN="avg(Petal_Length) as Petal_Length_avg, stdev(Petal_Width) as Petal_Width_stdev")[]
\end{Sinput}
\begin{Soutput}
           Petal_Length_avg Petal_Width_stdev
setosa                1.462         0.1053856
versicolor            4.260         0.1977527
virginica             5.552         0.2746501
\end{Soutput}
\end{Schunk}
The function creates and applies row and dimension names as required.
...
Use the new \verb+factor_scidb+ function to create R factor variables whose
levels are determined by a SciDB dimension array.
...
2         3.0          1.4         0.1  setosa
3         3.0          1.1         0.1  setosa
\end{Soutput}
\begin{Sinput}
...
\end{Sinput}
\begin{Soutput}
  Sepal_Width Petal_Length Petal_Width Species Species_index
1         3.5          1.4         0.2  setosa             0
2         3.0          1.4         0.2  setosa             0
3         3.2          1.3         0.2  setosa             0
4         3.1          1.5         0.2  setosa             0
5         3.6          1.4         0.2  setosa             0
6         3.9          1.7         0.4  setosa             0
\end{Soutput}
\end{Schunk}

See the on-line documentation for \verb+factor_scid+ for additional
examples.

\subsection{Missing values}

SciDB attribute values within a cell may be explicitly marked missing,
indicated by a special SciDB missing code also referred to as a \verb+NULL+
code.  SciDB internally supports a large number of possible missing codes. All
SciDB missing code values are mapped to \verb+NA+ values in R.
...
\end{Sinput}
\begin{Soutput}
[1] "redimension(subarray(redimension(between(transpose(test),3,null,3,null), <val:double NULL>[j=3:3,4,0,i=0:4,5,0]),null,null,null,null), <val:double NULL>[i=0:4,5,0])"
\end{Soutput}
\begin{Sinput}
> B@schema
\end{Sinput}
...
\end{Sinput}
\begin{Soutput}
[1] "R_array366d7fc6adf21102999947615"
\end{Soutput}
\begin{Sinput}
> C@schema
\end{Sinput}
...
The \verb+na.locf+ function is a generic function for replacing each missing or
empty value with the most recent non-missing value prior to it.

Unlike the usual \verb+na.locf+ function from the \verb+zoo+ package, the SciDB
\verb+na.locf+ function fills in both missing (SciDB \verb+null+ values) and
empty (SciDB sparse) values with the last non-missing and non-sparse value
along the indicated dimension.

Time series represented in SciDB are often sparse arrays. The \verb+na.locf+
...
then the output array can be huge. Consider using \verb+regrid+ first on
very fine time scales to reduce their resolution.

The default \verb+na.locf+ method in the \verb+zoo+ package unforunately
overrides this function (it uses \verb+ANY+ in its method signature). If
you need to use SciDB arrays and the \verb+zoo+ package, prefix SciDB's
version with \verb+scidb::na.locf(...)+.
...
# Set this to 32 for SciDB version 13.6!
  options(scidb.gemm\_chunk_size=1000)
...
# begin with "Rarray".
  options(scidb.safe\_remove=TRUE)
