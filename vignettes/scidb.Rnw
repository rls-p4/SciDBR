...
# Aggregate average speed by experiment using SciDB (using [] again):
aggregate(X, by="Expt", FUN="avg(Speed) as mean")[]
\end{lstlisting}\begin{lstlisting}[basicstyle={\ttfamily\footnotesize\color{blue}}]
  Expt_index  mean Expt
0          0 909.0    1
1          1 856.0    2
2          2 845.0    3
3          3 820.5    4
...
merge(a,b,by=list("surname","name"))[]
\end{lstlisting}\begin{lstlisting}[basicstyle={\ttfamily\footnotesize\color{blue}}]
   surname nationality deceased     name                         title
0   McNeil   Australia       no   McNeil     Interactive Data Analysis
1   Ripley          UK       no   Ripley            Spatial Statistics
2  Tierney          US       no  Tierney                     LISP-STAT
3    Tukey          US      yes    Tukey     Exploratory Data Analysis
4 Venables   Australia       no Venables Modern Applied Statistics ...
  other_author
0         <NA>
1         <NA>
2         <NA>
...
in R refering to the iris data and also a 1-D \verb+scidb+ array object in R
refering to the same data using the \verb+Sepal_Width+ attribute.
...
The \verb+scidb+ array class supports many standard linear algebra operations
