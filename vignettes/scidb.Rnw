...
Use the \verb+as.scidb+ function to create new SciDB
...
cell).  Consider the iris data presented earlier, represented within SciDB as a
...
\verb+Sepal_Width+ attribute.  Note that, unlike R, SciDB numeric array indices
...
in R and SciDB.  Similarly to data frames, the {\tt as.scidb} function can be
used to export R matrices and vectors to SciDB arrays.
...
# Compute the matrix vector product with an R-generated vector (in SciDB):
y = A %*% rnorm(5)
