...
\verb+Petal_Width+ attribute.  Note that, unlike R, SciDB array origin indices
are arbitrary and may be zero or negative (matrices and vectors in SciDB are
generally zero-indexed). Data from {\tt scidb} array objects are not
materialized to R until extracted with the empty indexing function, \verb+[]+.
Additional notes and examples about indexing appear in the next section.
...
\end{Sinput}
\begin{Soutput}
SciDB array name:  R_array3ec3b08cf9a11...
...
\begin{Soutput}
          [,1]        [,2]       [,3]        [,4]      [,5]
[1,] 10.438874  2.27018567 -7.1883979  1.04113352  4.274616
[2,]  2.270186  5.49588733 -0.6948195  0.05329931 -5.851780
[3,] -7.188398 -0.69481952  7.5489252 -2.70527379 -4.764550
[4,]  1.041134  0.05329931 -2.7052738  2.58124882  2.002071
[5,]  4.274616 -5.85177991 -4.7645497  2.00207129 12.071290
\end{Soutput}
\begin{Sinput}
...
function to interrogate the SciDB object for details.
...
Examples of indexing operations follow.
...
>
> # SciDB matrix objects like X default to zero-based indexing. It's important
> # that the label array have the same starting index:
...
\end{Soutput}
\end{Schunk}
\caption*{Indexing by a condition on an auxiliary array}
\end{rcode}
...
>
> # Create a centered version of the matrix by computing its column means with
> # apply and subtracting them with sweep. Note that apply uses a SciDB
> # expression. We specify `eval=TRUE` below to compute and cache the result of
> # sweep, improving the efficiency of the subsequent crossprod operation.
...
\begin{Schunk}
\begin{Sinput}
> # Upload a 5x5 matrix to SciDB. Its single numeric attribute name defaults to
> # 'val' (see help for as.scidb):
> set.seed(1)
...
limited cases, and most joins are limited to inner joins at the moment.
...
\end{Sinput}
\begin{Soutput}
[1] "sg(subarray(between(transpose(test),3,0,3,4),3,0,3,4),1,-1)"
\end{Soutput}
\begin{Sinput}
> B@schema
\end{Sinput}
...
\end{Sinput}
\begin{Soutput}
[1] "R_array3ec360a6e1cc1101127319914"
\end{Soutput}
\begin{Sinput}
> C@schema
\end{Sinput}
...
\begin{verbatim}
$                  crossprod      names          scidbrm
%*%                cumulate       names<-        sd
[                  df2scidb       ncol           show
acos               diag           nrow           sin
aggregate          diff           print          slice
apply              dim            project        sort
as.scidb           dimnames       redimension    str
asin               dimnames<-     regrid         subset
atan               head           rename         sum
attribute_rename   image          repart         svd
between            index_lookup   reshape        sweep
bind               iquery         rownames       t
build              is.scidb       rownames<-     tail
cast               is.scidbdf     scidb          tan
cbind              length         scidbconnect   tcrossprod
colnames           log            scidbeval      unique
colnames<-         max            scidblist      unpack
cos                merge          scidbls        var
count              min            scidbremove    xgrid
\end{verbatim}
