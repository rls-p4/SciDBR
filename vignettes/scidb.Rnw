...
SciDB array schema:  <Expt:int32 NULL DEFAULT NA,Run:int32 NULL DEFAULT NA,Speed:int32 NULL DEFAULT NA> [row=1:100,100,0]
Attributes:
...
3     Speed int32     TRUE
Row dimension:
  name  type start length chunk_interval chunk_overlap
1  row int64     1    100            100             0
\end{Soutput}
\begin{Sinput}
...
\end{Soutput}
\begin{Sinput}
> # Now create a vector-like object from the same SciDB array, but
> # referring to only one of the available variables (Petal_Width):
> x <- scidb(df, attribute="Petal_Width", data.frame=FALSE)
> length(x)
\end{Sinput}
\begin{Soutput}
[1] 150
...
\end{Sinput}
\begin{Soutput}
SciDB array name:  R_array458624d3ad101...
SciDB array schema:  <val:double NULL DEFAULT NA> [i=0:4,5,0,j=0:4,5,0]
...
\begin{Soutput}
           [,1]       [,2]       [,3]       [,4]       [,5]
[1,]  6.4246240  2.2260416  2.3209546 -0.0962403  4.1176475
[2,]  2.2260416  4.7946990  0.3261715  5.9363580 -0.7415525
[3,]  2.3209546  0.3261715  7.1619760 -1.8222962  2.0932342
[4,] -0.0962403  5.9363580 -1.8222962 21.3228324 -3.6040576
[5,]  4.1176475 -0.7415525  2.0932342 -3.6040576  4.8750848
\end{Soutput}
\begin{Sinput}
...
\end{Soutput}
\begin{Sinput}
> # Special SciDB-style maksed comparison:
> (x$Petal_Length %>% 1.4)[]
\end{Sinput}
\begin{Soutput}
...
\end{Sinput}
\begin{Soutput}
[1] "<val:double NULL DEFAULT NA> [i=0:4,5,0]"
\end{Soutput}
\begin{Sinput}
...
\end{Sinput}
\begin{Soutput}
[1] "R_array4586faea4c91100946420012"
\end{Soutput}
\begin{Sinput}
> C@schema
\end{Sinput}
...
$                 %<%               Filter            Ops
[                 abs               acos              aggregate
apply             as.scidb          asin              atan
attribute_rename  between           bind              build
cast              cbind             colnames          colnames<-
cos               count             crossprod         cumulate
df2scidb          diag              diff              dim
dimension_rename  dimensions        dimnames          dimnames<-
glm.fit           head              hist              image
index_lookup      iquery            is.scidb          is.scidbdf
lag               length            log               max
mean              median            merge             min
na.locf           names             names<-           ncol
nrow              print             project           redimension
...
show              sin               slice             solve
sort              str               subarray          subset
substitute        sum               svd               sweep
t                 tail              tan               tcrossprod
unique            unpack            var               xgrid
\end{verbatim}
