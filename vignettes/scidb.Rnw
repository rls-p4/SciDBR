...
{\bf Connect to SciDB on an encrypted port 8083 with example SciDB authentication:}
\begin{Schunk}
\begin{Sinput}
...
attribute separately. A one-dimensional SciDB array is conceptually similar to
a data frame in R: the SciDB dimension index corresponds to data frame row
index, and SciDB attributes to data frame columns.  Higher-dimensional arrays
in SciDB are generalizations of data frames and they don't correspond directly
to objects in R; the {\tt scidb} $n$-dimensional array class described below is
sometimes limited to working with one attribute at a time to more closely
emulate standard R arrays.
...
\end{Sinput}
\begin{Soutput}
SciDB expression:  R_array2c5342fad33f1102088094081
...
\begin{Soutput}
   j
j              0          1           2          3            4
  0  1.244033505 -0.7949141 -0.03214577  0.4985382 -0.007012987
  1 -0.794914112  1.6575862  0.15262576  2.4673273 -0.112623769
  2 -0.032145767  0.1526258  8.96206571  3.9547689 -0.019350146
  3  0.498538220  2.4673273  3.95476886  9.6855037 -0.375616327
  4 -0.007012987 -0.1126238 -0.01935015 -0.3756163  0.206898279
\end{Soutput}
\begin{Sinput}
...
The package adopts the convention that explicit subsets return with
starting coordinate indices defined by the subset operation,
noting also that zero and negative integers are valid SciDB indexes
...
\end{Soutput}
\end{Schunk}
Use the \verb+translate+ function to explicitly reset the starting
coordinate system of any SciDB array result to the SciDB default of
zero.
...
supported and SciDB always returns results in coordinate order.

If a sparse array has coordinates too large for R to manage, results are
returned in an ``unpacked'' data frame form.

SciDB objects with coordinates beyond the range of R display as
...
\end{Soutput}
\end{Schunk}
The package will attempt to rename variables to conform to SciDB
...
0         3.5          1.4         0.2  setosa 0.28
1         3.0          1.4         0.2  setosa 0.28
2         3.2          1.3         0.2  setosa 0.26
\end{Soutput}
\end{Schunk}
...
\begin{Soutput}
  Sepal_Width Petal_Length Petal_Width Species
0         3.1          1.5         0.1  setosa
1         3.0          1.4         0.1  setosa
2         3.0          1.1         0.1  setosa
\end{Soutput}
\begin{Sinput}
...
\end{Sinput}
\begin{Soutput}
[1] "redimension(redimension(between(transpose(test),3,null,3,null), <val:double NULL>[j=3:3,4,0,i=0:4,5,0]), <val:double NULL>[i=0:4,5,0])"
\end{Soutput}
\begin{Sinput}
> B@schema
\end{Sinput}
...
\end{Sinput}
\begin{Soutput}
[1] "R_array2c53209a026e1102088094081"
\end{Soutput}
\begin{Sinput}
> C@schema
\end{Sinput}
...
%<=%                    %==%                    %>=%
%*%                     +                       abs
acos                    aggregate               all.equal
antijoin                apply                   asin
as.scidb                atan                    attribute_rename
bernoulli               between                 bind
bound                   build                   c
cast                    cbind                   chunk_map
colnames                colnames<-              cos
count                   crossprod               cumulate
default                 df2scidb                diag
diff                    dim                     dimension_rename
dimensions              dimnames                dimnames<-
dist                    exp                     factor_scidb
Filter                  glm                     glm.fit
glm_scidb               %gt%                    head
hist                    image                   index_lookup
iqdf                    iquery                  is.scidb
is.scidbdf              is.temp                 kmeans
lag                     length                  levels_scidb
log                     %lt%                    max
mean                    median                  merge
min                     model_scidb             na.locf
names                   names<-                 ncol
nrow                    Ops                     order_scidb
peek                    persist                 phyper
